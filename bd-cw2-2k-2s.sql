...
VALUES ('Ноутбук Lenovo', 'Электроника', 45000.00),
...
SELECT
    cs.category,
    cs.total_sales,
    ROUND(cs.total_sales / cs.orders_count, 2) AS avg_per_order,
    ROUND(cs.total_sales / tsa.total_all * 100, 2) AS category_share
FROM category_sales cs, total_sales_all tsa
    ...
    GROUP BY 1, 2
     ...
     sales_with_lags AS (
         SELECT
             ms.year_month,
             ms.total_sales,
             LAG(ms.total_sales) OVER (ORDER BY ms.month_date) AS prev_month_sales,
                 LAG(ms.total_sales, 12) OVER (ORDER BY ms.month_date) AS prev_year_sales
